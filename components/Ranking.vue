        ...
        <v-card color="primary lighten-2">
            ...
            >
                                    ...
                                    <v-list dense subheader>
                                        ...
                                        <v-list-item
                                            :href="
                                                ...
                                                item.pedigreeRegistrationNumber
                                            "
                                            target="_blank"
                                        >
                                            <v-list-item-title
                                        ...
                                        <v-list-item
                                            :href="
                                                'https://db.netkeiba.com/horse/ped/' +
                                                item.pedigreeRegistrationNumber
                                            "
                                            target="_blank"
                                        >
                                            <v-list-item-title
                                                >血統</v-list-item-title
                                            >
                                        </v-list-item>
                                        <v-list-item
                                            :href="
                                                'https://db.netkeiba.com/?pid=horse_board&id=' +
                                                item.pedigreeRegistrationNumber
                                            "
                                            target="_blank"
                                        >
                                            <v-list-item-title
                                                >掲示板</v-list-item-title
                                            >
                                        </v-list-item>
                                    </v-list>
                                </v-menu>
                ...
                <template v-slot:[`item.point`]="{ item }">
                    <v-dialog width="400px" :fullscreen="isXs" scrollable>
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn
                        ...
                        <template #default="dialog">
                            <v-card>
                                ...
                                <v-card-text class="px-0">
                                    <v-card
                                        v-for="nomination in getDisplayNomination(
                                            item.nominationList
                                        )"
                                        :key="nomination.id"
                                        ...
                                        <v-list dense flat>
                                            <v-list-item-group>
                                                <v-list-item
                                                    v-for="(
                                                        raceResult, i
                                                    ) in getDisplayRaceResult(
                                                        nomination.raceResultList
                                                    )"
                                                    :key="i"
                                                >
                                                    <v-row>
    ...
    methods: {
        ...
        async get() {
            ...
            try {
                ...
                this.members = await this.$axios.$post(
                    '/get_owner_list_with_ranking',
                    params
                );
                // const res = await
        ...
        async getRanking() {
            ...
            this.ownerList = [];
            this.ownerList = await this.$axios.$post(
                '/get_owner_list_with_point',
                params
            );
            ...
            const ownerListWithNomination = await this.$axios.$post(
                '/get_nomination_list_by_group',
                params
            );
            ownerListWithNomination.forEach((value: Owner) => {
                const targetOwner = this.ownerList.find(
                    (owner: Owner) => owner.id === value.id
                );
        ...
        getDisplayNomination(nominationList: Nomination[]) {
            return nominationList.filter(
                (nomination) => nomination.point !== 0
            );
        },

        getDisplayRaceResult(raceResultList: RaceResult[]) {
            return raceResultList.filter(
                (raceResult) => raceResult.point !== 0
            );
        },
    },
});
